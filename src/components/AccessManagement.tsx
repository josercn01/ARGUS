...
export function AccessManagement() {
  ...
  const openEdit = (userItem: any) => {
    if (!checkPerm()) return;
    setEditingUser(userItem);
    setNovoPerfil(userItem.role);
    setShowEdit(true);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="bg-[#0b1329] border border-[#1e293b] p-4 rounded-2xl flex justify-between items-center">
        <div>
          <h2 className="text-white font-bold">🛡️ Gestão de Acessos do Sistema</h2>
          <p className="text-xs text-slate-400">Logado: {currentUserEmail} ({currentRole})</p>
        </div>
        <button onClick={() => setShowCreate(true)} className="bg-[#d4a017] hover:bg-yellow-400 text-black font-black text-xs px-4 py-2 rounded-xl flex gap-2 items-center cursor-pointer">
      ...
      <div className="bg-[#0b1329] border border-[#1e293b] rounded-2xl overflow-hidden">
        <div className="grid grid-cols-4 gap-4 p-4 text-[10px] uppercase text-slate-400 font-bold border-b border-white/5">
          <span>E-MAIL</span><span>PERFIL</span><span>CRIADO EM</span><span>AÇÕES</span>
        </div>
        <div className="max-h-[600px] overflow-y-auto">
          {loading ? <div className="p-10 text-center text-slate-400">Carregando...</div>
          : filtrados.map((a: any) => (
              <div key={a.email} className="grid grid-cols-4 gap-4 p-4 text-xs border-b border-white/5 hover:bg-white/5 text-slate-300 items-center">
                <span className="font-mono truncate">{a.email} {a.email === currentUserEmail && <span className="text-yellow-400 font-semibold">(você)</span>}</span>
                <span>
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase border ${a.role === 'super_admin' ? 'bg-red-500/10 text-red-400 border-red-500/20' : a.role === 'admin' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20'}`}>
                    {a.role}
                  </span>
                </span>
                <span>{a.created_at ? new Date(a.created_at).toLocaleString('pt-BR') : '-'}</span>
                <span className="flex gap-3">
                  <button onClick={() => openEdit(a)} className="text-yellow-400 hover:text-yellow-300 flex items-center gap-1 cursor-pointer">
                    <Pencil className="w-3 h-3" /> Editar
                  </button>
                  <button onClick={() => handleExcluir(a.email)} className="text-rose-400 hover:text-rose-300 disabled:opacity-30 cursor-pointer" disabled={a.email === currentUserEmail}>
                    Excluir
                  </button>
                </span>
              </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[#0b1329] border border-white/10 rounded-2xl w-full max-w-md">
            <div className="flex justify-between p-5 border-b border-white/5">
              <h3 className="text-white font-bold">Criar Usuário</h3>
              <button onClick={() => setShowCreate(false)} className="cursor-pointer"><X className="w-5 h-5 text-slate-400" /></button>
            </div>
            <form onSubmit={handleCriar} className="p-6 space-y-4">
              ...
              <button type="submit" disabled={saving} className="w-full bg-yellow-500 hover:bg-yellow-400 text-black font-black rounded-xl py-2.5 cursor-pointer">
      ...
      {showEdit && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[#0b1329] border border-white/10 rounded-2xl w-full max-w-md">
            <div className="flex justify-between p-5 border-b border-white/5">
              <h3 className="text-white font-bold flex gap-2 items-center"><Shield className="w-4 h-4 text-yellow-400" /> Editar Perfil</h3>
              <button onClick={() => setShowEdit(false)} className="cursor-pointer"><X className="w-5 h-5 text-slate-400" /></button>
            </div>
            <form onSubmit={handleEditar} className="p-6 space-y-4">
              <div>
                <label className="text-[11px] uppercase text-slate-400 font-bold">E-mail</label>
                <input disabled value={editingUser?.email} className="w-full mt-1 bg-black/50 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-slate-400" />
              </div>
              <div>
                <label className="text-[11px] uppercase text-slate-400 font-bold">Novo Perfil</label>
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setShowEdit(false)} className="flex-1 bg-[#1e293b] hover:bg-slate-700 text-white rounded-xl py-2.5 text-sm cursor-pointer">Cancelar</button>
                <button type="submit" disabled={saving} className="flex-1 bg-yellow-500 hover:bg-yellow-400 text-black font-black rounded-xl py-2.5 text-sm cursor-pointer">

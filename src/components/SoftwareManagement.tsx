...
export function SoftwareManagement({ softwares, onClose, onRefresh }: Props) {
  ...
  async function handleSave() {
    ...
    if (isAdobe) {
      ...
      if (!nomeCustom.trim()) {
        alert('Digite o nome do software ex: AutoCAD');
        setSaving(false);
        return;
      }
    ...
    const payload = {
      ...
      qtd_contratada: qtdFinal,
      familia: familiaLegacy
    };
      ...
      <div className="bg-[#001E33] border border-[#1e293b] rounded-xl w-full max-w-[560px] max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex justify-between items-center p-5 border-b border-[#1e293b]">
          <h3 className="text-white font-bold flex items-center gap-2">
            <Layers className="w-5 h-5 text-[#D4AF37]" /> Gerenciar Softwares - Cadastro
          </h3>
              ...
              <div className="space-y-3">
                ...
                {tipoAdobe === 'SINGLE'? (
                  <>
                    ...
                    {isPoolSingle? (
                      <input type="number" value={qtd} onChange={e => setQtd(Number(e.target.value))} placeholder="Qtd do Pool ex: 225" className="w-full bg-[#001E33] border border-[#1e293b] rounded-lg px-3 py-2.5 text-white text-sm" />
                    ) : (
                      <>
                        ...
                        <p className="text-[11px] text-[#64748b]">App individual com qtd 0, consome 1 do Pool Single Apps.</p>
                      </>
                    )}
                  </>
                ) : (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-[#64748b]">
                      {s.is_adobe? `Adobe - ${s.tipo_adobe}` : 'Não Adobe'} {s.qtd_contratada > 0? ` - ${s.qtd_contratada} licenças` : ' - consome do pool'}
                    </p>
                  </div>
                </div>

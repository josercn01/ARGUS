...
import { Pencil, Trash2, Upload, Download, X, CheckCircle, AlertCircle, UserPlus } from 'lucide-react';
...
`;
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>){
    ...
    if(missing.length>0){
      setStatus('error');
      setLogs(prev=>[...prev, `ERRO: Colunas faltando: ${missing.join(', ')}`]);
      return;
    }

    // --- DIFERENCIAL: NÃO DUPLICA ---
    ...
    const novosMap = new Map<string, any>(); // dedup dentro da planilha
    ...
    for(let i=1;i<lines.length;i++){
      ...
      if(!email) continue;
      if(emailsExistentes.has(email)) continue; // já existe no banco
      if(novosMap.has(email)){ duplicadosNaPlanilha.count++; continue; } // duplicado dentro da planilha
      novosMap.set(email, {
    ...
    setLogs(prev=>[
     ...prev,
      `${totalLinhas} linhas na planilha`,
      `${emailsExistentes.size} usuários já cadastrados`,
      `${jaExistiam} já existiam (ignorados)`,
      `${listaNovos.length} NOVOS para adicionar`,
     ...(duplicadosNaPlanilha.count? [`${duplicadosNaPlanilha.count} duplicados dentro da planilha (ignorados)`] : []),
    ]);

    if(listaNovos.length === 0){
      setProgress(100);
      setStatus('success');
      setIgnorados(jaExistiam);
      setLogs(prev=>[...prev, 'Nenhum usuário novo para adicionar.']);
      return;
    }
    ...
    try{
      for(let i=0;i<listaNovos.length;i++){
        ...
        const pct = 30 + Math.round((i / listaNovos.length) * 65);
        setProgress(pct);
        ...
        for(const t of tipos){
          const { data: soft } = await supabase.from('softwares').select('id,nome').ilike('nome', `%${t}%`).limit(1).maybeSingle();
          if(soft && userId){
        ...
        setLogs(prev=>[...prev.slice(-15), `+ ${row.email_original} | ${row.nome}`]); // mantém últimos 15 no log
      }

      setAdicionados(adicionadosTemp);
      setIgnorados(jaExistiam);
      setProgress(100);
      setStatus('success');
      setLogs(prev=>[...prev, `✓ Concluído: ${adicionadosTemp.length} novos adicionados, ${jaExistiam} ignorados (já existiam)`]);
      onRefresh?.();
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <p className="text-[10px] text-[#64748b]">Não duplica: só adiciona diferenças | Formato ;</p>
      </div>
          ...
          <div className="bg-[#001E33] border border-[#1e293b] rounded-xl w-full max-w-[550px] p-5">
            <div className="flex justify-between mb-4">
              <h3 className="text-white font-bold flex items-center gap-2">
                ...
                {status==='importing' || status==='parsing'? 'Importando...' : status==='success'? `Importação concluída` : 'Erro'}
                {status==='success' && adicionados.length>0 && <span className="text-emerald-400 text-sm">+{adicionados.length} novos</span>}
              </h3>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-xs text-[#94a3b8]">
                <span>{progress}%</span>
                <span>{status==='success'? `${adicionados.length} adicionados | ${ignorados} já existiam` : ''}</span>
              </div>
              ...
              {adicionados.length > 0 && status==='success' && (
                <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-lg p-3">
                  <p className="text-xs font-bold text-emerald-400 mb-2 flex items-center gap-1"><UserPlus className="w-4 h-4"/> Usuários adicionados ({adicionados.length}):</p>
                  <div className="max-h-40 overflow-auto space-y-1">
                    {adicionados.map((a, idx)=>(
                      <div key={idx} className="text-[11px] text-white bg-[#001726] rounded px-2 py-1 flex justify-between">
                        <span className="truncate">{a.email_original}</span>
                        <span className="text-[#94a3b8] truncate ml-2">{a.nome}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {status==='success' && adicionados.length===0 && (
                <div className="bg-sky-500/10 border border-sky-500/20 rounded p-3 text-xs text-sky-300">
                  Nenhum usuário novo. Todos os {ignorados} emails da planilha já estavam cadastrados.
                </div>
              )}

              <div className="flex justify-end gap-2 pt-2">
                <button onClick={()=>{ setShowImport(false); setStatus('idle'); setProgress(0); }} className="px-4 py-2 bg-[#0f172a] border border-[#1e293b] rounded text-white text-xs">Fechar</button>
              </div>
            </div>
          </div>
        </div>

...
export function FiltersBar({
  ...
  const temFiltro = Boolean(search || software || local || status || departamento);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-3">
      ...
      <div className="relative">
        <select value={departamento} onChange={(e) => onDepartamentoChange(e.target.value)} className={`${selectClass} min-w-[190px]`}>
      ...
      <div className="relative">
        <select value={software} onChange={(e) => onSoftwareChange(e.target.value)} className={selectClass}>
          ...
          {(softwares || []).map((s) => (
            <option key={s.id} value={s.id}>{softwareLabel(s)}</option>
          ))}
        </select>
        <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-4 h-4 text-[#64748b] pointer-events-none" />
      </div>
      ...
      <div className="relative">
        <select value={status} onChange={(e) => onStatusChange(e.target.value)} className={`${selectClass} min-w-[150px]`}>
      ...
      {temFiltro && (
        <button onClick={limpar} title="Limpar filtros" className="flex items-center justify-center gap-2 text-sm text-[#94a3b8] hover:text-[#D4AF37] border border-[#1e293b] hover:border-[#D4AF37]/40 rounded-lg px-3 py-2.5 transition-all cursor-pointer">

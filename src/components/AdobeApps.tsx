    ...
    <div className="bg-[#020C1A] min-h-screen p-6 text-white -m-8">
      <div className="max-w-[1100px] mx-auto">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] px-2 py-1 rounded bg-[#FF0000]/20 border border-[#FF0000]/30 text-red-300 flex items-center gap-1">
                <ShieldCheck className="w-3 h-3" /> @senado - SYNC
              </span>
              {syncing && <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse" />}
            </div>
        ...
        <div className="bg-[#0a1930] border border-white/10 rounded-xl overflow-hidden">
          <div className="grid grid-cols-12 px-5 py-3 text-[11px] font-semibold text-zinc-400 uppercase border-b border-white/10">
            ...
            return (
              <div key={p.id} className="grid grid-cols-12 px-5 py-[14px] items-center border-b border-white/[0.05] hover:bg-white/[0.03]">
                <div className="col-span-6 flex items-center gap-3">
                  <div className="w-8 h-8 rounded bg-gradient-to-br from-[#FF0000] to-[#FF6A00] flex items-center justify-center text-[12px] font-black">A</div>
                  <span className="text-[13px] truncate">{p.nome}</span>
                </div>
                <div className="col-span-2 text-[13px]">
                  <span className="font-bold">{p.atribuidas}</span>
                  <span className="text-zinc-400"> de {p.total}</span>
                  <div className={`text-[11px] ${disp < 10 ? 'text-amber-400' : 'text-emerald-400'}`}>{disp} disponíveis</div>
                </div>
                <div className="col-span-1">
                  {p.tag && <span className="text-[10px] px-2 py-1 rounded bg-white/10 border border-white/20">{p.tag}</span>}
                </div>

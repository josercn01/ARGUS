...
export function MetricsCards({ data, softwares }: { data: UsuarioLicenca[], softwares: Software[] }) {
  const stats = useMemo(() => {
    ...
    const emUso = Array.from(countBySoftware.values()).reduce((a,b)=>a+b, 0);
    ...
    const detalhe = softwares.filter(s=>s.qtd_contratada>0).map(balde => {
      ...
      if (balde.tipo_adobe==='SINGLE' || balde.familia==='SINGLE_POOL') {
        softwares.filter(s=>s.qtd_contratada===0 && s.is_adobe).forEach(s => {
    ...
    return { total, emUso, livres, taxa, detalhe };
  }, [data, softwares]);

  return (
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-[#001E33] border border-[#1e293b] rounded-xl p-4">
          <p className="text-xs text-[#94a3b8]">TOTAL CONTRATADO</p>
          <p className="text-2xl font-bold text-white mt-2">{stats.total}</p>
        </div>
        <div className="bg-[#001E33] border border-[#1e293b] rounded-xl p-4">
          <p className="text-xs text-[#94a3b8]">EM USO</p>
          <p className="text-2xl font-bold text-emerald-400 mt-2">{stats.emUso}</p>
        </div>
        <div className="bg-[#001E33] border border-[#1e293b] rounded-xl p-4">
          <p className="text-xs text-[#94a3b8]">DISPONIVEIS</p>
          <p className="text-2xl font-bold text-sky-400 mt-2">{stats.livres}</p>
        </div>
        <div className="bg-[#001E33] border border-[#1e293b] rounded-xl p-4">
          <p className="text-xs text-[#94a3b8]">TAXA DE UTILIZACAO</p>
          <p className="text-2xl font-bold text-[#D4AF37] mt-2">{stats.taxa}%</p>
        </div>
      </div>
        ...
        {stats.detalhe.map(b => (
          <div key={b.nome} className="bg-[#001726] border border-[#1e293b] rounded-xl p-3">
            ...
            <div className="flex justify-between items-end mt-1">
              ...
              <p className="text-xs font-bold text-sky-400">{b.livre} livres</p>
            </div>
            <div className="w-full bg-[#001E33] h-1.5 rounded mt-2">
              <div className="bg-[#D4AF37] h-1.5 rounded" style={{width: `${Math.min(100, b.contratado? b.usado/b.contratado*100 : 0)}%`}}></div>
            </div>
          </div>
        ))}
      </div>
